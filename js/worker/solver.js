...
const MIN_SEG   = 0.3;        // min output segment length, px
...
function computeCornerNormals(nv, nt, tri, pos, fn, ea, eb, et0, et1, eang, hardEdgeDeg){
...
function intersectSegs(ax,ay,bx,by,cx,cy,dx,dy){
...
function buildShadowMap(pos, tri, fn, nt, L, watertight, eps){
...
function worldOnFace(f, px, py, tri, pos, sxA, syA, izA, ortho){
  const a=tri[f*3], b=tri[f*3+1], c=tri[f*3+2];
...
// per texture type.
function buildPatternSegsFromTest(testFn, cx, cy, spacing, corners){
  let maxDist = 0;
...
function pairJunctionArms(arms){
  const n = arms.length;
...
function dedupCollinear(arr, offTol=DEDUP_OFF_TOL, gapTol=DEDUP_GAP_TOL){
...
function subtractCovered(loArr, hiArr, offTol=DEDUP_OFF_TOL, gapTol=DEDUP_GAP_TOL){
...
function flipBufferRowsY(buf, w, h){
  const rowFloats = w * 4;
...
function sampleShading(buf, w, h, sx, sy){
