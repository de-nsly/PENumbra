/* ================================================================
   ...
   preview and the real export, renderPaper() (re-lays out the
   on-screen SVG render-result.js builds), and the final
   export-to-.svg-file handler.
   The two export-time geometry passes — dash splitting and "Trim SVG
   export to margins" — are path-model.js, which treats the Circles
   layer's cubic arcs as curves throughout and never flattens them to
   polylines. Export SVG has two modes, picked by the
   ...
   If a line ends up in the wrong place on the page, chain.js is the
   main-thread suspect; worldOnFace()/intersectSegs()/subtractCovered
   are the worker's.
   ================================================================ */
import { $, DASH_KEYS, DASH_RATIOS, MAX_DASH_SLOTS, PEN_LIBRARY, SVG_NS, dashPattern, downloadFile, penById, scaledDash, svgEl } from './main.js';
...
import { activeTab, gatherSettings, lastGen, makeSliderValueEditable, markStale, syncLineLayerUI, updateGroundPatternSliderRange } from './panel-controls.js';
import { renderTextureStack } from './texture-stack.js';
import { blockLayerPenId, blocks, computeLayoutPaperDims, createBlockDom, gridGuidePositions, layoutOverlayOn, refreshAllBlockStyles, renderPreviewLayoutOverlay, syncLayoutPaperFrame, syncLayoutTrimMask, updateBlockStyle } from './layout-canvas.js';
