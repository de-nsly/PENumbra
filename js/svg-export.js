/* ================================================================
   ...
   flattens them to polylines. Both run on the export CLONE only: the
...
function splitDashedPathD(d, dashLen, gapLen){
  ...
  for (const sp of parsePathD(d)){
    ...
    for (const seg of segs){
      ...
      const L = tbl.total;
      if (L > 1e-9){
        for (let k = Math.floor(pos/period); ; k++){
          const onStart = k*period, onEnd = onStart + dashLen;
          ...
          if (cur && Math.abs(s - curEnd) < 1e-9) cur.segs.push(sub.seg);   // same "on" run continuing across a segment join
